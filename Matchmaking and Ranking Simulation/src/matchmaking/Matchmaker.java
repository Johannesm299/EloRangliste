...
/**
 * Matchmaking ist die Klasse in der alle Spieler gespeichert sind, die nach
 * Spielen suchen, sowie alle Spiele, die derzeit noch nicht voll sind und noch
 * Spieler benötigen.
 * 
 * 
...
public class Matchmaker implements Runnable {
	 ...
	 * 
	 * @param p
	 *            Spieler der die Queue/das Matchmaking betritt
	 */
	...
	/**
	 * Ordnet einen Spieler einem Match zu und entfernt ihn von den Spielsuchenden
	 * Spielern.
	 * 
	 * @param p
	 *            Spieler der zugeordnet wird
	 * @param m
	 *            Match dem der Spieler zugeteilt wird
	 * @param k
	 *            Index des Spielers, der einem Spiel zugeteilt wird
	 * @param j
	 *            Index des Spiels dem der Spieler zugeteilt wird
	 */
	...
	/**
	 * Erstellt ein neues Match, nachdem kein geeignetes für einen Spieler gefunden
	 * wurde.
	 * 
	 * @param j
	 *            der Spieler fuer den ein neues Match erstellt wird
	 */
	 ...
	 * 
	 * @param i
	 *            die Obergrenze an zu spielenden Matches
	 */
	...
	/**
	 * Durchsucht die Spieler und Matches nach passenden Spielern fuer ein Match,
	 * anhand des gameRanks der Matches und des Rangs des Spielers. Lässt ein Match
	 * starten, sobald es voll ist. Dokumentiert alle gespielten Spiele im
	 * Workplace. Stoppt, wenn die Obergrenze erreicht ist.
	 */
	public void run() {
				...
				if (matchesPlayed <= intervallOG) {
					for (int i = lowerm; i < upperm; i++) {
						if (players.get(j).getRank().getEloNumber() >= 1500.0) {
							if (players.get(j).getRank().getEloNumber() >= 0.9 * matches.get(i).getGameRank()
									&& players.get(j).getRank().getEloNumber() <= 1.1 * matches.get(i).getGameRank()) {
						...
						if (players.get(j).getRank().getEloNumber() < 1500.0
								&& players.get(j).getRank().getEloNumber() >= 1000.0) {
							if (players.get(j).getRank().getEloNumber() >= 0.8 * matches.get(i).getGameRank()
									&& players.get(j).getRank().getEloNumber() <= 1.2 * matches.get(i).getGameRank()) {
								playerPlaysMatch(players.get(j), matches.get(i), j, i);
								foundGame = true;
								break;
							}
						...
						if (players.get(j).getRank().getEloNumber() < 1000.0) {
							if (players.get(j).getRank().getEloNumber() >= 0.7 * matches.get(i).getGameRank()
									&& players.get(j).getRank().getEloNumber() <= 1.3 * matches.get(i).getGameRank()) {
