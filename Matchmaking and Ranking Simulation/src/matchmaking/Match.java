...
/**
 * Match ist die Klasse in der zwei Teams gegeneinander antreten.
 * Es werden solange Runden gespielt, bis ein Team 16 Runden gewonnen hat, oder 30 Runden gespielt wurden.
 * 
...
public class Match implements Runnable {
	 ...
	 * 
	 * @param p initialer Spieler, der den anfaenglichen gameRank des Matches bestimmt.
	 */
	public Match(Player p) throws FileNotFoundException, UnsupportedEncodingException{
		teamOne[0]=p;
		...
		iD=iDcounter++;
		writer = new PrintWriter("Match " + iD, "UTF-8");
	}
	/**
	 * Fuegt einen Spieler diesem Match hinzu.
	 * 
	 * @param p der zu hinzufügende Spieler
	 */
	public void addPlayer(Player p){
		if(averageRatingTone>=averageRatingTtwo){
			if(numPlayersTtwo!=5){
				...
				averageRatingTtwo=(((averageRatingTtwo*numPlayersTtwo)+p.getRank().getEloNumber()+0.0)/(numPlayersTtwo+1.0));
				numPlayersTtwo++;
				gameRank=(averageRatingTone*numPlayersTone+averageRatingTtwo*numPlayersTtwo+0.0)/(numPlayersTone+numPlayersTtwo+0.0);
			}
				...
				averageRatingTone=(((averageRatingTone*numPlayersTone)+p.getRank().getEloNumber()+0.0)/(numPlayersTone+1.0));
				numPlayersTone++;
				gameRank=(averageRatingTone*numPlayersTone+averageRatingTtwo*numPlayersTtwo+0.0)/(numPlayersTone+numPlayersTtwo+0.0);
			}	
		}
	 ...
	 * @return true, wenn das Match bereits gestartet hat und noch nicht beendet ist. Ansonsten false
	 */
	 ...
	 * 
	 * @return true, wenn die Anzahl der Spieler in diesem Match =10 ist. Ansonsten false.
	 */
	...
	/**
	 * Schreibt die Spieler, sowie deren Daten, beider Teams in das Textdokument fuer dieses Match.
	 */
	public void printTeams(){
		writer.println("//Team 1: ");
		//System.out.println("//Team 1: ");
		for(int i=0;i<teamOne.length;i++){
			//System.out.println(teamOne[i].getName() + " S: " + teamOne[i].getPlayerSkill() + " v: " + teamOne[i].getPlayerVolatility() + " RS: " + playerSkill[i]);
			writer.println(teamOne[i].getName() + " S: " + teamOne[i].getPlayerSkill() + " v: " + teamOne[i].getPlayerVolatility() + " RS: " + playerSkill[i]);
		}
		writer.println("///////////////////////");
		...
		for(int i=0;i<teamTwo.length;i++){
			//System.out.println(teamTwo[i].getName() + " S: " + teamTwo[i].getPlayerSkill() + " v: " + teamTwo[i].getPlayerVolatility() + " RS: " + playerSkill[i+5]);
			writer.println(teamTwo[i].getName() + " S: " + teamTwo[i].getPlayerSkill() + " v: " + teamTwo[i].getPlayerVolatility() + " RS: " + playerSkill[i+5]);
		}
	...
	/**
	 * Laesst beide Teams solange gegeneinander Spielen, bis eins 16 Runden gewonnen hat, oder 30 Runden gespielt wurden.
	 * Die gespielten Runden werden dokumentiert. Nach dem Spiel werden die Raenge der Spieler angepasst, sowie die Rangveraenderung dokumentiert.
	 * Anschließend betreten die Spieler dieses Spiels wieder die Warteschlange/Matchmaker.
	 */
	public void run(){
		printTeams();
		...
		for (int i=0;i<r.length;i++){
			...
			if(k==1){
				...
				writer.println("Team1 won Round" + getCurrentRound() + " Es steht Team1: " + roundsT1 + " Team2: " + roundsT2);
				//System.out.println("Team1 won Round" + getCurrentRound() + " Es steht Team1: " + roundsT1 + " Team2: " + roundsT2);
			}
			else if(k==2){
				roundsT2++;
				String vl [] =r[i].getRundenverlauf();
				...
				writer.println("Team2 won Round" + getCurrentRound()+ " Es steht Team1: " + roundsT1 + " Team2: " + roundsT2);
				//System.out.println("Team2 won Round" + getCurrentRound()+ " Es steht Team1: " + roundsT1 + " Team2: " + roundsT2);
			}
			else if(k==25){
		...
		for(int j=0;j<5;j++){
			stats[j].getPlayer().getRank().adjustRanking(sumRangpunkteT1,sumRangpunkteT2,1,winner,roundsT1,roundsT2,stats[j].getKills()*2, sumPT1);
			if(winner==1){
		...
		for(int j=5;j<10;j++){
			stats[j].getPlayer().getRank().adjustRanking(sumRangpunkteT1,sumRangpunkteT2,2,winner,roundsT1,roundsT2,stats[j].getKills()*2, sumPT2);
			if(winner==2){
				stats[j].getPlayer().increaseWonGames();
				stats[j].getPlayer().increaseGames();
			}
